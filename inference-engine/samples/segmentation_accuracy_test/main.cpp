...
#include "segmentation_accuracy_test.h"
...
int main(int argc, char *argv[]) {
    try {
        ...
        SizeVector outDims = outputBlob->getTensorDesc().getDims();

        for (auto i : outDims)
            std::cout << " " << std::to_string(i);
        std::cout << "\n";

        auto outData = outputBlob->buffer().as<PrecisionTrait<Precision::FP32>::value_type *>();

        for (int c = 0; c < 3; c++)
        {
            std::cout << "channel " << std::to_string(c) << ":\n";
            for (int h = 0; h < 10; h++)
            {
                for (int w = 0; w < 10; w++)
                    std::cout << " " << std::to_string(outData[c * 256 * 256 + h * 256 + w]);
                std::cout << "\n";
            }
            std::cout << "\n";
        }
